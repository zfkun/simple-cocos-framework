const { readFileSync: readFileSync } = require("fs"),
  ...
  { spawnSync: spawnSync } = require("child_process"),
...
try {
  const e = readFileSync(PATH.packageJSON, "utf8"),
    o = JSON.parse(e).devDependencies["@cocos/creator-types"].replace(
      /^[^\d]+/,
      ""
    );
  checkCreatorTypesVersion(o) ||
    (console.log("[33mWarning:[0m"),
    console.log("  @en"),
    console.log("    Version check of @cocos/creator-types failed."),
    console.log(
      `    The definition of ${o} has not been released yet. Please export the definition to the ./node_modules directory by selecting "Developer -> Export Interface Definition" in the menu of the Creator editor.`
    ),
    console.log(
    ...
    console.log(
      `    ${o} 定义还未发布，请先通过 Creator 编辑器菜单 "开发者 -> 导出接口定义"，导出定义到 ./node_modules 目录。`
    ),
